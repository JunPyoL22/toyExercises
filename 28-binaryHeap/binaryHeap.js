...
function BinaryHeap (comparisonFunc) {
  this._heap = [];
  // this compare function will result in a minHeap, use it to make comparisons between nodes in your solution
...
BinaryHeap.prototype.insert = function (value) {
  ...
  this.heapify(value, this._heap.length - 1)
}

BinaryHeap.prototype.removeRoot = function () {
  // TODO: Your code here
  const deleteEleInArr = (arr, index) => {
    for(let i=index; i<this._heap.length-1; i++) {
      arr[i] = arr[i+1] 
      this.heapify(arr[i], i)
    }
    arr.pop()
  }
  ...
  deleteEleInArr(this._heap, index)
}

BinaryHeap.prototype.heapify = function (value, index) {

  const swapInArr = (arr, indexA, indexB) => {
    let temp = arr[indexA]
    arr[indexA] = arr[indexB] 
    arr[indexB] = temp 
  }

  const subRoutine = (value, index) => {
    ...
    if(!this._compare(this._heap[parentIndex], value)) {

      swapInArr(this._heap, parentIndex, index)

      // if(this._heap[childIndexLeft] && this._heap[childIndexRight]) {
      //   if(!this._compare(this._heap[childIndexLeft], this._heap[childIndexRight])) {
      //     swapInArr(this._heap, childIndexLeft, childIndexRight)
      //   }
      // } 
      subRoutine(value, parentIndex)
    }
  }
  subRoutine(value, index) 
}

const bHeap = new BinaryHeap((i, j) => i<j) 
// bHeap.insert(3)
// bHeap.insert(5)
// bHeap.insert(2)
// bHeap.insert(1)
// bHeap.insert(4)
// console.log(bHeap._heap)
