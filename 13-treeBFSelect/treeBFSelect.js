...
Tree.prototype.BFSelect = function(filter) {
  ...
  const _searchChildFunc = (tree) => {
    ...
    for (let key in tree.children) {
      if (filter(tree.children[key].value, depth)) {
        resultArr.push(tree.children[key].value);
      }
      footPrintArr.push(tree.children[key]);
    }

    for (let key in footPrintArr) {
      _searchChildFunc(footPrintArr[key]);
    }
    return ;
  };
