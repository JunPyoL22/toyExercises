...
const shuffleDeck = deck => {
  // Your code here
  const swichInDeck = (indexA, indexB) => {
    let temp = deck[indexA]
    deck[indexA] = deck[indexA+indexB]
    deck[indexA+indexB] = temp
  }
  ...
  for (let i=maxNumber; i>0; i--) {
    ...
    swichInDeck(maxNumber-i, randomNumber)
  }
  return deck
}
...
//test
deckO = orderedDeck()
console.log('original deck and length:', deckO, deckO.length)
deckS = shuffleDeck(deckO)
console.log('shuffled deck and length:', deckS, deckS.length)
