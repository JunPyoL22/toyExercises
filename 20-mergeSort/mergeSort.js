...
const merge = (leftArr, rightArr) => {
  ...
  while (leftDone && rightDone) {
    if (leftArr[leftIndex] <= rightArr[rightIndex])  {
      storage.push(leftArr[leftIndex]);
      leftIndex ++;
    } else if (leftArr[leftIndex] > rightArr[rightIndex]) {
      storage.push(rightArr[rightIndex]);
      rightIndex ++;
    } 
    leftDone = leftIndex < leftArr.length;
    rightDone = rightIndex < rightArr.length;
  }
  if (!leftDone && rightDone) {
    for (let i=rightIndex; i<rightArr.length; i ++) {
      storage.push(rightArr[i]);
    }
  } else {
    for (let i=leftIndex; i<leftArr.length; i ++) {
      storage.push(leftArr[i]);
    }
  }
  return storage;
};
...
//test
console.log(merge([], [2,6,9]));
console.log(merge([5, 6, 9, 14], [4, 10, 18]));
console.log(mergeSort([5, 6, 9, 14, 4, 10, 18]));
console.log(mergeSort([31, 9 ]));
